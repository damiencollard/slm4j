...
object StsValidator {
  ...
  case class SignatureMatch(text: StsText) extends SignatureVerification
...
class StsValidator {
  ...
  private def readSignedText(fileName: String, publicKey: PublicKey, textMarker: String): Try[StsSignedText] =
    for (lines     <- readLines(fileName);
         textLines <- extractText(lines, textMarker);
         sig       <- extractSignature(lines, publicKey))
      yield StsSignedText(textLines, sig)
  ...
  def verifySignedText(signedText: StsSignedText, publicKey: PublicKey): Try[Boolean] =
    Try {
